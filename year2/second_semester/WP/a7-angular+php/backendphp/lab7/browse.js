...
function get_filtered_by_genre() {
  ...
  ajax.onreadystatechange = function () {
    if (this.readyState === 4 && this.status === 200) {
      ...
      for (let i = 0; i < json.length; i++) {
        ...
        Object.keys(file1).forEach(function (k) {
          let text;
  ...
  ajax.open("POST", "getAllFilesByGenre.php", true);
...
function setPrevious(filt, option) {
  document.getElementById("previous-filter").innerHTML =
    "Previously used: " + filter + " filter: " + opt;
  filter = filt;
  opt = option;
}
