    ...
    def read_tokens(self):       
        with open(r"D:\uni\UniversityProjects\year3\S1\FLCD\L3\lab2_python\token.in","r") as file:
            for line in file:
                ...
                if parts[0] in ["FCT","int","string","char","bool","READ","IF","ELSE","DO","arr","GO_THROUGH","SENDBACK","SHOW","SENDBACK","IF","MIN_INT","MAX_INT","WHILE","terminal","print"]:
    ...
    def tokenize(self,tokensToBe):
        ...
        for t in filtered_tokensToBe:
            if ( t[0] == '"'):
                ...
                isCharConstant = not isCharConstant
                continue
...
# 4 - separators
    ...
    def scan(self, program_file_name):
        try:
            ...
            for entry in tokens:
                ...
                if ( token in self.reservedWords):
                    ...
                    lexical_error_exists.append(f"Error at line {locationPair[0]} and column {locationPair[1]}, invalid token {token} \n")
        ...
        if ( len(lexical_error_exists) == 0) :
            ...
            with open("PIF" + program_file_name +".txt", "w") as file_writer:
                for pair in self.PIF:

                    
                    file_writer.write(f"token: {pair[0][0]} --at: {pair[0][1]}  type-> ({pair[1]})\n")

            with open("ST" + program_file_name +".txt", "w") as file_writer:
